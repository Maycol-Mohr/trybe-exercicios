...
function oldBooks() {
  ...
  //return books.filter((book) => anoAtual2 - book.releaseYear > 60).map((book) => book.name);
}
...
function oldBooksOrdered() {
  ...
  return books.filter((book) => (book.releaseYear < anoAtual - 60)).sort((booka, bookb) => booka.releaseYear - bookb.releaseYear);
}
...
function formatedBookNames() {
  return books.map((informacoes) => `${informacoes.name} - ${informacoes.genre} - ${informacoes.author.name}`);
...
//const verifyAgeDrive = (arrayOfPeople) => (
  //arrayOfPeople.filter((people) => (people.name === 'Vitor'))
//);
...
const listProducts = [{ Arroz: 2.99 }, {Feijao: 3.99}, {Alface: 1.5}, {Tomate: 2}];
*/
...
numeros.filter((elemento, index, array) => console.log(elemento, index, array));
*/
